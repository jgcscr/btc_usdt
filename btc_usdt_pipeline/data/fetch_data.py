        ...
        for col in numeric_columns:
            if col in df.columns:
                ...
                if not non_numeric.empty:
                    logger.warning(f"Found {len(non_numeric)} non-numeric values in '{col}' column: {non_numeric.head(5).tolist()}")
                
                # Convert to numeric with coerce to handle errors
                df[col] = pd.to_numeric(df[col], errors='coerce')
                
                # Fill NaN values with forward fill, then backward fill as fallback
                df[col] = df[col].ffill().bfill()
                
                if df[col].isna().any():
                    logger.warning(f"Column '{col}' still has {df[col].isna().sum()} NaN values after filling")
        
        logger.info(f"Dtypes after conversion: {df.dtypes}")
        
        # Detect if we're running in Google Colab (check if path contains 'drive')
        is_gdrive = 'drive' in str(raw_data_path).lower()
        
        try:
            # Use pyarrow engine explicitly when saving to parquet
            ...
            try:
                ...
                logger.error(f"All save attempts failed. Last error: {e2}")
